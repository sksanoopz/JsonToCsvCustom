package com.dailycodechallenge.jsontocsv.jsontocsv.util;
...
public class JFlat {
    ...
    private Map<String,String> mapperFile;
    ...
    public JFlat(String jsonString,Map<String,String> mapperFile,String[] csvHeaders) {
        ...
        if (ele.isJsonObject()) {
            ...
            for (Map.Entry<String, JsonElement> entry : ele.getAsJsonObject().entrySet()) {
                ...
                if (entry.getValue().isJsonPrimitive()) {
                    ...
                    if ((mapperFile.get(tmpPath)) != null
                            && unique.contains(mapperFile.get(containsInMap(unique,tmpPath))))
                    {
                       // System.out.println("Temp path"+tmpPath );
                        //System.out.println("Value"+entry.getValue().getAsJsonPrimitive() );
                        int index = unique.indexOf(mapperFile.get(tmpPath));
    ...
    private String containsInMap(List<String> unique, String tmpPath) {
       return  mapperFile.keySet().stream()
                .filter(e -> e.toUpperCase().endsWith(tmpPath.toUpperCase()))
                .findFirst()
                .orElse(null);
    }
